...
- 采用函数形式返回的是一个全新的data对象，不会指向同一个内存地
## vue中给对象添加新属性界面没反应
...
- 首先新老节点的开始和结束位置进行标记，然后进入循环寻找相同的节点，相同的就复用，再根据具体情况移动新老节点的索引，新老节点的索引都是从中间向两边靠拢，当新节点或者老节点的startIndex 大于 endIndex就结束循环，再根据新老节点的数目进行创建或移除
## v-for循环中的key作用
- 在用 v-for 渲染列表的数据来睡，当数据量很大，我们还要对这个数据增删改的操作，如果给里边增加一条数据，整个列表就要重新渲染这样性能开销太大了。
